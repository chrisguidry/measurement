...
class QuantityTestCase(unittest.TestCase):
    ...
    def testImmutability(self):
        ...
        try:
            ...
            assert str(e) == "Quantity is immutable."
        else:
    ...
    def testParsingNonsense(self):
        ...
        except MeasurementParsingException, e:
            assert str(e) == "Could not parse 'foobar' to a Quantity.", str(e)
        else:
    ...
    def testStringConversionForIntegers(self):
        ...
        assert str(10 * Ten) == "100", str(10 * Ten)
        
    def testArithmeticAxiomsOverLongs(self):
        arithmetic.axioms.additive_identity = Quantity(long(0), One)
        arithmetic.axioms.distributable_with = Quantity(long(4), One)
        arithmetic.axioms.hold_for(Quantity(long(10), One))
        
        arithmetic.axioms.additive_identity = Quantity(long(0), Meter)
        arithmetic.axioms.distributable_with = Quantity(long(4), Meter)
        arithmetic.axioms.hold_for(Quantity(long(4), Meter))
        
        arithmetic.axioms.additive_identity = Quantity(long(0), Meter / Second)
        arithmetic.axioms.distributable_with = Quantity(long(4), Meter / Second)
        arithmetic.axioms.hold_for(Quantity(long(5), Meter / Second))
        
        arithmetic.axioms.additive_identity = Quantity(long(0), Ohm)
        arithmetic.axioms.distributable_with = Quantity(long(4), Ohm)
        arithmetic.axioms.hold_for(Quantity(long(6), Ohm))
        
    def testStringConversionForLongs(self):
        string_representations.should_represent_orthogonally(Quantity(long(10), One))
        string_representations.should_represent_orthogonally(Quantity(101231937212349871928471234, One))
        string_representations.should_represent_orthogonally(Quantity(long(4), Meter))
        string_representations.should_represent_orthogonally(Quantity(101231937212349871928471234, Meter))
        string_representations.should_represent_orthogonally(Quantity(long(5), Meter / Second))
        string_representations.should_represent_orthogonally(Quantity(101231937212349871928471234, Meter / Second))
        string_representations.should_represent_orthogonally(Quantity(long(6), Ohm))
